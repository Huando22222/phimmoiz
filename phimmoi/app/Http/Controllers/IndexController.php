...
class IndexController extends Controller
{
    ...
    public function category($slug){
        ...
        // Khai báo biến $sort_by trước khi sử dụng
        $sort_by = 'desc'; // Mặc định sắp xếp theo ngày cập nhật giảm dần
    
        // Kiểm tra xem biến $sort_by đã được gửi từ yêu cầu hay chưa
        if(isset($_GET['sort_by'])){
            $sort_by = $_GET['sort_by'];
        }
    
        // Truy vấn dữ liệu phim với sắp xếp
        if($sort_by == 'giam_dan'){
            $movie = Movie::withCount('episode')->where('category_id',$cate_slug->id)->orderBy('ngaycapnhat','DESC')->paginate(40)->appends(request()->query());
        } elseif($sort_by == 'tang_dan'){
            $movie = Movie::withCount('episode')->where('category_id',$cate_slug->id)->orderBy('ngaycapnhat','ASC')->paginate(40)->appends(request()->query());
        } elseif($sort_by == 'kytu_za'){
            $movie = Movie::withCount('episode')->where('category_id',$cate_slug->id)->orderBy('title','ASC')->paginate(40)->appends(request()->query());
        }elseif($sort_by == 'kytu_za'){
            $movie = Movie::withCount('episode')->where('category_id',$cate_slug->id)->orderBy('title','DESC')->paginate(40)->appends(request()->query());
        }else {
            $movie = Movie::withCount('episode')->where('category_id',$cate_slug->id)->orderBy('ngaycapnhat','DESC')->paginate(40);
        }           
    
        return view('pages.category', compact('cate_slug','movie','title','description','image','sort_by'));
    }
